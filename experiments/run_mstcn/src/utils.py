...
def load_meta(data_root, model_root, result_root, record_root, dataset, split, model_name, epoch_level_augmentation = False):
    ...
    if epoch_level_augmentation:
        gt_path = (os.path.join(data_root, dataset, 'groundTruth/'), os.path.join(data_root, dataset, 'groundTruthProbability/'))
    else:
        gt_path = os.path.join(data_root, dataset, 'groundTruth/')
    if epoch_level_augmentation:
        features_path =  (os.path.join(data_root, dataset, 'features/'), os.path.join(data_root, dataset, 'i3d/'))
    else:
    ...
    vid_list_file_val = os.path.join(data_root, dataset, 'splits', 'val.split' + str(split) + '.bundle')
    ...
    return actions_dict, num_actions, gt_path, features_path, vid_list_file, vid_list_file_val, vid_list_file_tst, sample_rate, model_dir, result_dir, record_dir
...
def eval_txts(data_root, result_dir, dataset, split, name, mode, epoch):
    ground_truth_path = os.path.join(data_root, dataset, 'groundTruth/') 
    recog_path = os.path.join(result_dir)
    if mode == "validation":
        file_list = os.path.join(data_root, dataset, 'splits', 'val.split'+str(split)+'.bundle')
    elif mode == "training":
        file_list = os.path.join(data_root, dataset, 'splits', 'train.split'+str(split)+'.bundle')
    elif mode == "testing":
        file_list = os.path.join(data_root, dataset, 'splits', 'test.split'+str(split)+'.bundle')
    else:
        assert "Please select a valid mode"

    list_of_videos = read_file(file_list).split('\n')[:-1]

    if mode == "training":
        list_of_videos = list(filter(lambda x: "_original" not in x, list_of_videos))
    ...
    for vid in list_of_videos:
        ...
        recog_file = os.path.join(recog_path, mode, str(epoch), vid.split('.txt')[0])
